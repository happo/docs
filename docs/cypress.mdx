...
import ApiKeySecurityNote from './_partials/api-key-security-note.mdx';
import SkippingSnapshots from './_partials/skipping-snapshots.mdx';
import Support from './_partials/support.mdx';
import WhereAreMyScreenshots from './_partials/where-are-my-screenshots.mdx';
import EmailNotifications from './_partials/email-notifications.mdx';
import StylesMissing from './_partials/styles-missing.mdx';
```
  ...
  cypress:
    jobs:
      ...
      - cypress/run:
          ...
          parallelism: 4
          cypress-command: |
            npx happo --nonce ${CIRCLE_WORKFLOW_ID} -- npx cypress run
          post-steps:
...
<EmailNotifications framework="Cypress" />
...
<WhereAreMyScreenshots />

<StylesMissing module="happo/cypress" />
