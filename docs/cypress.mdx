...
with `cypress open`). To enable Happo in these scenarios, set
  ...
  cypress:
    jobs:
      ...
      - cypress/run:
          ...
          parallel: true
          parallelism: 4
          command-prefix: 'HAPPO_NONCE=${CIRCLE_WORKFLOW_ID} npx happo -- npx'
          post-steps:
            - run: 'HAPPO_NONCE=${CIRCLE_WORKFLOW_ID} npx happo finalize'
```
