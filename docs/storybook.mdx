...
below, and it is required on older versions of the `happo` package, which did
not ship the runtime inside the package.
...
- Getting `Timed out while waiting for window.happo`? The error names the most
  ...
  what each one means. On older versions of the `happo` package, the usual fix
  is adding `import 'happo/storybook/register'` to your `.storybook/preview.js`
  file.
